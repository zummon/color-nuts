...
<div class="">
  <div class="flex flex-row gap-2">
    {#each chooseknots as knot}
      <span class="h-16 w-8 rounded-full" style="background-color: {knot || '#9ca3af'};"></span>
    {:else}
...
<div class="flex flex-wrap gap-4 px-4">
  {#each screws as knots, index (index)}
    <button class="flex flex-col gap-2" onclick={() => {
      let ready = []
      let position
      if (chooseknots[0]) {
      } else {
        ...
        chooseknots = ready
        screws[index].splice(position, ready.length, ...ready.slice().fill(''))
      }
    }}>
      {#each knots as knot, idx (`${index}-${idx}`)}
        <span class="h-8 w-16 rounded-full" style="background-color: {knot || '#9ca3af'};"></span>
      {/each}
    </button>
  {/each}
</div>